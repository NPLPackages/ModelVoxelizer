...
local BlockModel = commonlib.inherit(nil,commonlib.gettable("Mod.ModelVoxelizer.bmax.BlockModel"));
...
function BlockModel:Scale(scale_x,scale_y,scale_z)
	for _, vertice in pairs(self.m_vertices) do
		if(vertice.position)then
			vertice.position:MulByVector({scale_x,scale_y,scale_z});
		end
	end
end
